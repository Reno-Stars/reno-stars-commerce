...
import previous from './data/eurofit-before-v3-hashes.json'
...
export default async function updateEurofit({container}: ExecArgs) {
 ...
 for(const p of inventory.products){
  ...
  const revised=true
  const metadata={...prior.metadata,...(revised?{model_url:'https://supply.reno-stars.com'+p.model_url,model_sha256:p.sha256,model_version:3,model_profile:p.model_profile,model_limitations:p.model_limitations}: {})}
 ...
 const knobs=inventory.products
