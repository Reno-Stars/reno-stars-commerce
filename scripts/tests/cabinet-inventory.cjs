...
function importer(service,workflow,fixture=data){const module={exports:{}};const ts=require('../../storefront/node_modules/typescript');const code=ts.transpileModule(fs.readFileSync(path.join(root,'backend/src/scripts/import-cabinet-inventory.ts'),'utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS,esModuleInterop:true}}).outputText;const mocks={'./data/cabinet-inventory.json':fixture,'@medusajs/core-flows':{createProductsWorkflow:workflow},'@medusajs/framework/utils':{ContainerRegistrationKeys:{LOGGER:'logger'},Modules:{PRODUCT:'product',SALES_CHANNEL:'sales'},ProductStatus:{PUBLISHED:'published'}}};vm.runInNewContext(code,{module,exports:module.exports,require:id=>mocks[id],process,fetch:async()=>({ok:true,headers:{get:()=> 'model/gltf-binary'}}),AbortSignal});return ()=>module.exports.default({container:{resolve:k=>({logger:{info(){}},product:service,sales:{listSalesChannels:async()=>[{id:'sc'}]}}[k])}})}
