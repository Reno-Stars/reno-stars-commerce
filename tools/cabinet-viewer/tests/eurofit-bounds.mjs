...
for(const p of inventory.products.filter(p=>p.model_status==='built')){
 const file=process.env.EUROFIT_AUDIT_DIR ? path.join(process.env.EUROFIT_AUDIT_DIR,'models',p.id+'.glb') : new URL('../storefront/public'+p.model_url,root);const b=fs.readFileSync(file);const g=await new GLTFLoader().parseAsync(b.buffer.slice(b.byteOffset,b.byteOffset+b.byteLength),'');
...
fs.writeFileSync(process.env.EUROFIT_AUDIT_DIR ? path.join(process.env.EUROFIT_AUDIT_DIR,'bounds-audit.json') : new URL('eurofit-hardware/glb-validation.json',root),JSON.stringify(results,null,2)+'\n');console.log(`Validated ${results.length} GLB mesh envelopes and mounting anchors.`);
