...
const port=process.env.NATIVE_VIEWER_PORT||'8770';const server=spawn(process.execPath,[path.join(storefront,'node_modules/next/dist/bin/next'),'dev','-p',port],{cwd:storefront,stdio:['ignore',log,log],env:{...process.env,NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY:'pk_local_test',NEXT_PUBLIC_MEDUSA_BACKEND_URL:'https://supply-admin.reno-stars.com'}});
async function main(){
 try{
  let ready=false;for(let i=0;i<120;i++){try{const r=await fetch(`http://127.0.0.1:${port}/cabinet-library/catalog.json`,{signal:AbortSignal.timeout(3000)});if(r.ok){ready=true;break}}catch{}await new Promise(r=>setTimeout(r,1000));}
  if(!ready)throw Error('Next test server did not become ready');
  const test=spawn(process.execPath,[path.join(__dirname,'native-product.cjs')],{cwd:repo,stdio:'inherit',env:{...process.env,NATIVE_VIEWER_TEST_URL:`http://127.0.0.1:${port}/api/cabinet-viewer-test`}});
